...
class WatchdogPro:
    ...
    def check_level2(self, pid):
        ...
        if now - self.last_tick_time >= 60:
            ...
            print(f"[{self.pkg_name}] Level 2 Delta: {delta} ticks")
    ...
    def is_alive(self):
        pid = self.get_pid()
        if not pid:
            print(f"[{self.pkg_name}] Level 1: Process not found.")
            return False
        
        if not self.check_level2(pid):
            print(f"[{self.pkg_name}] Level 2: Soft freeze detected (low ticks).")
            return False
            
        return True

    def force_stop(self):
        print(f"Force stopping {self.pkg_name}...")
        subprocess.run(f"su -c 'am force-stop {self.pkg_name}'", shell=True)
        self.freeze_counters = {"level2": 0}
        self.last_ticks = 0

if __name__ == "__main__":
    print("WatchdogPro module loaded.")
